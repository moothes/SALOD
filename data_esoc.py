...
def get_image_list(name, config):
    ...
    for idx in range(5):
        ...
        f = open('{}/ESOC/subset{}.txt'.format(config['data_path'], idx), 'r')
        for line in f.readlines():
            images.append(os.path.join(image_root, 'ESOC', 'image', line.strip() + '.jpg'))
            gts.append(os.path.join(image_root, 'ESOC', 'mask', line.strip() + '.png'))
...
def get_loader(name, config=None):
    dataset = ESOC_Train('ESOC', config)
...
class ESOC_Train(data.Dataset):
    def __init__(self, name, config):
        ...
        for idx in range(5):
            ...
            f = open('{}/ESOC/subset{}.txt'.format(config['data_path'], idx), 'r')
            for line in f.readlines():
                self.images.append(os.path.join(config['data_path'], 'ESOC', 'image', line.strip() + '.jpg'))
                self.gts.append(os.path.join(config['data_path'], 'ESOC', 'mask', line.strip() + '.png'))
...
class ESOC_Val:
    def __init__(self, name, config=None):
        ...
        f = open('{}/ESOC/subset{}.txt'.format(config['data_path'], test_split), 'r')
        for line in f.readlines():
            self.image_paths.append(os.path.join(config['data_path'], 'ESOC', 'image', line.strip() + '.jpg'))
            self.gt_paths.append(os.path.join(config['data_path'], 'ESOC', 'mask', line.strip() + '.png'))
            
        #self.image_paths, self.gt_paths = get_image_list(name, config)
        self.size = len(self.image_paths)
        
        bar = Bar('Dataset {:10}:'.format('ESOC'), max=self.size)
